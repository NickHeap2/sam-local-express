...
function startServer (serverConfig) {
  servers = []
  const { apis, globalEnvVars, globalTimeout, globalCors, parameters } = serverConfig
  ...
  if (globalParameters.singlePort) {
    ...
    for (const api of apis) {
      console.log(`  Configuring ${api.type} ${api.name} stage ${api.stage}...`.green)
      listenPort = globalParameters.basePort + apiNumber
      apiNumber += 1
...
function checkAuthOptions (api) {
  // disable auth?
  if (globalParameters.disableAuth) {
