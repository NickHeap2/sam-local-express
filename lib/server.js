...
const { apis, globalEnvVars, globalTimeout, globalCors } = parseResult
...
function newExpress (api, combined) {
  ...
  let combinedText = combined ? '(from first API)' : ''
    ...
    app[route.method](invokePath, async (req, res) => {
      if (auth.hasAuthorizer) {
        ...
        if (typeof authResponse === 'string') {
          ...
          return
        } else if (auth.simpleResponses === false && (!authResponse.policyDocument?.Statement?.[0] || authResponse.policyDocument.Statement[0].Effect !== 'Allow')) {
